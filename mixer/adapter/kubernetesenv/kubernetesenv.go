...
import (
	...
	"istio.io/istio/pkg/kube/clusterregistry"
)
...
// Required for unit test override. RACC - probably don't need this for dynamic implementation
...
func (b *builder) Build(ctx context.Context, env adapter.Env) (adapter.Handler, error) {
	...
	if !found {
		if err := initMultiClusterController(b, path, env); err != nil {
...
func (b *builder) kubeCreateController(k8sInterface k8s.Interface, clusterID string) error {
	controller, err := getNewCacheController(b, k8sInterface, b.kubeHandler.env)
	if err == nil {
...
func (b *builder) kubeDeleteController(clusterID string) error {
...
func initMultiClusterController(b *builder, kubeconfig string, env adapter.Env) (err error) {
	kubeClient, err := newKubernetesClient(kubeconfig, env)
	if err != nil {
		return fmt.Errorf("could not create K8s client: %v", err)
	}

	err = clusterregistry.StartSecretController(kubeClient, b.kubeCreateController, b.kubeDeleteController, "istio-system")
	if err != nil {
